...
const CLIENT_ID = "1528225096253046784";
...
async function setActivity(data) {
    ...
    await client.user.setActivity({
        type: 2,
        details: data.episode,
        largeImageKey: data.image || "spotify",
        largeImageText: data.show,
        startTimestamp: data.playing ? new Date(start) : undefined,
        endTimestamp: data.playing ? new Date(end) : undefined,
        buttons: [{ label: "Open on Spotify", url: data.url }],
        instance: false
    });
}
